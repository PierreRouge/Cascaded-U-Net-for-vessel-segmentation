...
from monai.transforms import LoadImaged, AddChanneld, EnsureTyped, NormalizeIntensityd, RandSpatialCropd, adaptor, SqueezeDimd, Compose, EnsureChannelFirstd
...
from utils.utils_training import train_loop_newskel
...
parser.add_argument("--num_iter", type=int, default=5, help="Num iter for skeletonization")
...
print(f"Epoch {1}\n-------------------------------")
epoch_save = 1
logs = train_loop_newskel(train_data, val_data, model=model, input_='MRI', optimizer=optimizer, device=device, epoch=1, max_epoch=epochs, alpha_=alpha, beta=beta, tau=tau, method=method, num_iter=num_iter)

loss = logs['train_loss']
loss_dice = logs['train_loss_dice']
loss_cldice = logs['train_loss_cldice']
val_loss = logs['val_loss']
val_loss_dice = logs['val_loss_dice']
...
file_training.write("loss:" + str(loss) + "loss_dice:" + str(loss_dice) + "loss_cldice:" + str(loss_cldice) + ',val_loss:' + str(val_loss) + ',val_dice:' + str(val_dice) + ',time:' + str(epoch_duration) + '\n')
file_training.close()
...
for t in range(1, epochs):
    ...
    logs = train_loop_newskel(train_data, val_data, model=model, input_='MRI', optimizer=optimizer, device=device, epoch=t+1, max_epoch=epochs, alpha_=alpha, beta=beta, tau=tau, method=method, num_iter=num_iter)
