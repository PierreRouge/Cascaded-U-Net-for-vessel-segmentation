...
parser.add_argument('--K', metavar='K', type=int, nargs="?", default=8, help='Weight for Dice loss in Dice + clDice loss')
...
res = dir_res + '/unet/' + args.job_name + '_' + str(num)
dir_exist = 0
while dir_exist != 1:
...
for (root, directory, file) in os.walk(dir_inputs):
    for f in file:
        ...
        if split[0] in patient_test:
            data_test.append(dict(zip(['image', 'GT', 'DTM', 'filename'], [dir_inputs + '/' + f, dir_GT + '/' + name + '_GT.nii.gz', dir_GT.replace('GT', 'DistanceMap') + '/' + name + '_DistanceMap.nii.gz', f])))
